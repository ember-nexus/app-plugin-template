...
class TaskStatesView extends LitElement {
  ...
  private handleInputChange = (prop: 'newTaskStateName' | 'newTaskStateColor') => (e: Event) => {
  ...
  private renderTaskState(taskState: TaskState, index: number): TemplateResult {
    return html`
      <li class="p-3 flex gap-2 ${index < this.taskStates.length - 1 ? 'border-b border-gray-300' : ''}">
        ...
        <button class="btn btn-square btn-ghost" title="edit task status">
  ...
  render(): TemplateResult {
    return html`
      <div class="flex flex-col gap-2 p-3">
        ...
        ${this.renderCreateTaskStateDialog()}
      </div>
    `;
  }
}
